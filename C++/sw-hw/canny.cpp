...
canny::canny(String filename)
{
    ...
    cout << "\nHw input gen time: " << hwInputGen.elapsedTime() << endl;
    ...
    cout << gFiltered.rows << " " << gFiltered.cols << endl;
    cout << angles_dave.rows << " " << angles_dave.cols << endl;

    for(int i = 0; i < gFiltered.rows; i++){
    	for(int j = 0; j < gFiltered.cols; j++){
    		
    		//the size of angles_dave is the 2 less rows and 2 less cols than gFiltered
    		if(i == 0 || j == 0 || i == gFiltered.rows - 1 || j == gFiltered.cols - 1){ 
    			outputFile << gFiltered.at<uint16_t>(i,j) << ",0" << endl;
    		}else{
    			outputFile << gFiltered.at<uint16_t>(i,j) << "," << (int16_t)angles_dave.at<float>(i-1,j-1) << endl;
    		}
