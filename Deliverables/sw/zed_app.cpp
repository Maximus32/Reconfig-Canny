...
/*
	for(unsigned i = 0; i < my_count; i++){
		cout << hw_inputs[i] << endl;
	}
*/
	//cout << "\nDebug spot 3 reached" << endl;

	//declare the timers
	Timer writeTime, readTime, waitTime;

	//cout << "\nDebug spot 5 reached \n";

	//write values to the FPGA
	writeTime.start();
	board->write(hw_inputs, MEM_IN_ADDR, data_size);
	board->write(&num_rows, ROWS_ADDR, 1);
	//cout << "\nDebug spot 6 reached" << endl;
	board->write(&num_cols, COLS_ADDR, 1);
	board->write(&data_size, SIZE_ADDR, 1);
	//cout << "\nDebug spot 7 reached" << endl;
	//cout << "\nDebug spot 8 reached" << endl;
	...
	//cout << "\nDebug spot 9 reached" << endl;

	while(!done){
		board->read(&done, DONE_ADDR, 1);
	}

	//cout << "\nDebug spot 10 reached" << endl;
	...
	board->read(hwOutput, MEM_OUT_ADDR, data_size);
	readTime.stop();

	//cout << "\nDebug spot 11 reached" << endl;
	double transferTime = writeTime.elapsedTime() + readTime.elapsedTime();
	double hwWithCalcTime = waitTime.elapsedTime() + hw_input_gen_time;

	printf("%d,%d,%f,%f\n", num_rows-2, num_cols-2, transferTime, hwWithCalcTime);
